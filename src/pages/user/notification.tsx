...
import {BookingInput, BookingList, getBooking} from "@/services/booking.service";
...
const Notification = () => {
    ...
    const getTime = (booking: BookingList) => {
        ...
        hourOfStart = startHour < 10 ? '0' + startHour.toString() : startHour.toString();
        hourOfEnd = endHour < 10 ? '0' + endHour.toString() : endHour.toString();
        minutesOfStart = startMinutes < 10 ? '0' + startMinutes.toString() : startMinutes.toString();
        minutesOfEnd = endMinutes < 10 ? '0' + endMinutes.toString() : endMinutes.toString();
        return (
            <div
                className="text-gray-500">{hourOfStart}.{minutesOfStart} - {hourOfEnd}.{minutesOfEnd}</div>
        );
    }
    return (
        <>
            <main className="w-full h-screen flex bg-backgroundColor pt-24 h-full min-h-screen flex-col">
                ...
                <div className="flex-col flex justify-center w-full">
                    <div
                        className="grid sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-10 justify-center items-center p-5"
                    >
                        {booking?.map((booking) => {
                            return (
                                <div
                                ...
                                >
                                    ...
                                    <div
                                        className="text-gray-500">{booking.startDateTime.getDate()}/{booking.startDateTime.getMonth()}/{booking.startDateTime.getFullYear()}</div>
                                    {getTime(booking)}
